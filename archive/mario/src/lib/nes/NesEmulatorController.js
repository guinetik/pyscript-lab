...
import { initializeHeadlessNESFactory } from './HeadlessNES.js';
...
export class NesEmulatorController {
	...
	constructor(canvasElement) {
		...
		// ROM data (stored for HeadlessNES cloning)
		...
		// Initialize headless NES factory (for background training)
		initializeHeadlessNESFactory();
	...
	async initialize(romPath, onReady, onError) {
		...
		try {
			...
			// Create speakers - no buffer underrun handling
			// Generating extra frames on underrun speeds up the game which breaks training
			// Accept audio glitches rather than incorrect game speed
			this.speakers = new Speakers({});

			// Create NES instance
			this.setupNES();

			// Create frame timer at native 60 FPS
			// Agent still observes every frame but game runs at correct speed
			this.frameTimer = new FrameTimer({
				fps: 60,  // Run at native NES speed
				onGenerateFrame: (time) => {
					if (this.nes && this.isLoaded) {
						this.nes.frame();

						// Notify Python agent every frame
						if (this.frameSync) {
	...
	async loadROM(path) {
		try {
			...
			// Store ROM for HeadlessNES cloning
			this.romBinaryString = binaryString;
