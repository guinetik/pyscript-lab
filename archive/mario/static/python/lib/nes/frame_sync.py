"""
Frame Synchronization Manager - Ensures agent observes at consistent rate

Provides a callback-based interface that guarantees the agent observes
at a fixed rate, preventing timing drift and missed states.

The NES emulator runs at native 60 FPS for smooth audio.
The agent is notified every 2 emulator frames (30 observations/second).
This gives smooth audio while keeping agent processing manageable.
"""
...
class FrameSyncManager:
    """
    Manages synchronization between NES emulator and agent observations.

    Uses a callback mechanism where JavaScript notifies Python at a fixed rate.
    Emulator runs at 60 FPS for smooth audio, agent observes at 30 FPS.

    The 2:1 ratio (60 FPS emulator → 30 FPS observations) is controlled by JS.
    """
