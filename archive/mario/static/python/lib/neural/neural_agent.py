...
class MarioAgent(NeuralAgent):
    ...
    def observe(self) -> np.ndarray:
        ...
        # Row encoding (one-hot of Mario's vertical position, clamped to valid range)
        if self.encode_row:
            _, mario_row = self.game.get_mario_tile_position()
            # Clamp row to valid range (screen has ~15 tile rows, but we encode 0 to num_rows-1)
            # This ensures Mario on ground (row 10+) still gets encoded (as bottom row)
            clamped_row = max(0, min(mario_row, self.num_rows - 1))
