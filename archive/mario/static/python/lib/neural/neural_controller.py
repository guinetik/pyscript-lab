...
class SimpleNeuralController(NeuralController):
    ...
    def randomize(self):
        """Randomize all network weights using uniform[-1, 1] (matching reference)."""
        for i in range(len(self.network.weights)):
            self.network.weights[i] = np.random.uniform(-1, 1, size=self.network.weights[i].shape).astype(np.float32)
            self.network.biases[i] = np.random.uniform(-1, 1, size=self.network.biases[i].shape).astype(np.float32)
...
class ActionDecoder:
    ...
    MATCHING REFERENCE: Simple > 0.5 threshold, let network learn.
    ...
    def __init__(self, use_variable_threshold: bool = False, max_buttons: int = 3, simple_controls: bool = False):
        ...
        Args:
            use_variable_threshold: Ignored (kept for compatibility)
            max_buttons: Ignored (reference doesn't limit buttons)
    ...
    def decode(self, output: np.ndarray) -> np.ndarray:
        """
        Convert network output to button presses using SIMPLE > 0.5 THRESHOLD.
        MATCHING REFERENCE: No conflict resolution, no top-k, just threshold.

        The network learns through evolution which combinations work best.

        Button order (6-button): [UP, DOWN, LEFT, RIGHT, A, B]
        Button order (4-button): [LEFT, RIGHT, A, B] → expanded to [0, 0, LEFT, RIGHT, A, B]

        Args:
            output: Neural network output (sigmoid activations 0-1)
        ...
        # Simple > 0.5 threshold
        buttons = (output_flat > 0.5).astype(int)

        # CONFLICT RESOLUTION: LEFT (idx 2) and RIGHT (idx 3) cancel each other!
        # If both are pressed, only keep the stronger one
        # This helps networks learn without getting stuck at walls
        LEFT_IDX, RIGHT_IDX = 2, 3
        if buttons[LEFT_IDX] == 1 and buttons[RIGHT_IDX] == 1:
            # Both directions pressed - pick the stronger activation
            if output_flat[LEFT_IDX] > output_flat[RIGHT_IDX]:
                buttons[RIGHT_IDX] = 0
            else:
                buttons[LEFT_IDX] = 0
    ...
    def decode_deterministic(self, output: np.ndarray) -> np.ndarray:
        """
        Same as decode() - reference uses same logic everywhere.
