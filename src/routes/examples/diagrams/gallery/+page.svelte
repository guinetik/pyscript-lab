...
<ExperimentCard props={{ previousPage: '/examples/bokeh/communities', nextPage: '/examples/diagrams/create' }}>
	...
	<article slot="content_slot" class="mb-10">
		<h2 class="mb-5 text-xl font-heading font-bold text-text-primary">{$exampleText.title || 'Diagrams Gallery'}</h2>

		<div class="space-y-4">
			<Callout>
				<h3 class="mb-2 font-heading font-bold">{$exampleText.sections?.whatIs?.title || '📦 What is Diagrams?'}</h3>
				<p class="text-sm">
					<a
						href="https://diagrams.mingrammer.com"
						target="_blank"
						class="font-bold text-accent underline hover:opacity-80"
			...
			<Callout>
				<h3 class="mb-2 font-heading font-bold">{$exampleText.sections?.syntax?.title || '🔗 Simple Syntax'}</h3>
				<p class="text-sm">
					The library uses intuitive Python syntax with operators like
					<code class="rounded bg-surface-alt px-1">{$exampleText.sections?.syntax?.dataFlow || '>> '}</code>
					{$exampleText.sections?.syntax?.description?.split('for data flow,')[0] || ''}for data flow,
					<code class="rounded bg-surface-alt px-1">{$exampleText.sections?.syntax?.connections || ' - '}</code>
					for connections, and list notation
					<code class="rounded bg-surface-alt px-1">{$exampleText.sections?.syntax?.parallel || '[]'}</code>
					for parallel components. Clusters group related resources together.
				</p>
			</Callout>

			<Callout type="tip">
				<h3 class="mb-2 font-heading font-bold">{$exampleText.sections?.browserMagic?.title || '🎉 Browser Magic with viz.js!'}</h3>
				<p class="text-sm">
					...
					<code class="bg-surface-alt px-1 rounded">{$exampleText.sections?.browserMagic?.digraph || 'graphviz.Digraph'}</code> class to capture DOT
					output.
				</p>
