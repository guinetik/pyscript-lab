	...
	<div slot="py_slot" class="flex h-full w-full flex-col p-6">
		...
		<div class="mb-4 flex-shrink-0">
			...
			<textarea
				...
				rows="5"
				class="w-full rounded-lg border-2 border-border bg-surface p-4 text-base text-black shadow-sm focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20"
		...
		<div class="flex-1 overflow-auto">
			...
			{#if updateMessage}
				...
				>
					<p class="text-3xl mb-2">{updateMessage.type === 'success' ? 'Done' : '...'}</p>
...
{$exampleText.ui?.noButton || '✗ NO'}
			...
			{#if isInitializing && !error}
				<div class="rounded-lg bg-surface-alt p-4 text-center border-2 border-border">
